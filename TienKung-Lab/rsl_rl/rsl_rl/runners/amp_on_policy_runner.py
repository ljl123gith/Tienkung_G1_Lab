...
import rsl_rl
from rsl_rl.algorithms import AMPPPO,AMPPPO_WGAN_GP
...
from rsl_rl.utils import AMPLoader,AMPLoader_25, Normalizer, store_code_state
...
class AmpOnPolicyRunner_25:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        # resolve training type depending on the algorithm
        # 添加所有AMP相关算法的支持
        if self.alg_cfg["class_name"] in ["PPO", "AMPPPO", "AMPPPO_WGAN_GP"]:
            self.training_type = "rl"
        elif self.alg_cfg["class_name"] == "Distillation":
            self.training_type = "distillation"
        else:
        ...
        with torch.no_grad():
            env_amp_obs = self.env.get_amp_obs_for_expert_trans()
        print(
            f"[DEBUG][AMP] expert_dim={amp_data.observation_dim}, "
            f"env_amp_dim={env_amp_obs.shape[1]}"
        )
        ########
        amp_normalizer = Normalizer(amp_data.observation_dim )	# TODO:Normalizer(amp_data.observation_dim - 8)
        # Select discriminator based on algorithm class name
        # NOTE:
        #   - We only import `Discriminator` (baseline MLP) and `Discriminator_WGAN_GP` (WGAN-GP / pairwise style).
        #   - Old names like `Discriminator_Pairwise` / `Discriminator_baseline` were left in comments/code but are
        #     not actually defined or imported, which caused a NameError at runtime.
        #   - Here we map algorithms to the *existing* classes to avoid such runtime issues.
        alg_class_name = self.alg_cfg.get("class_name", "AMPPPO")

        # 1) Pairwise AMP variant → 使用 WGAN-GP 判别器（结构是 pairwise 的）
        if alg_class_name == "AMPPPO_Pairwise":
            print(
                f"[DEBUG][AMP][Runner] Using Discriminator_WGAN_GP as pairwise discriminator "
                f"(alg_class_name={alg_class_name}, input_dim={amp_data.observation_dim * 2})"
            )
        ...
        # 2) Explicit WGAN-GP 算法 → 同样使用 WGAN-GP 判别器
        elif alg_class_name == "AMPPPO_WGAN_GP":
            print(
                f"[DEBUG][AMP][Runner] Using Discriminator_WGAN_GP for AMPPPO_WGAN_GP "
                f"(input_dim={amp_data.observation_dim * 2})"
            )
            discriminator = Discriminator_WGAN_GP(
                amp_data.observation_dim * 2,
                train_cfg["amp_reward_coef"],
        ...
        # 3) 默认 / 基线 AMPPPO → 使用原版 Discriminator（即 baseline 判别器）
        else:
            # 这里原来写的是 `Discriminator_baseline`，但这个类既没有定义也没有导入，
            # 会在运行时触发 NameError。实际想要的是“普通的 AMP 判别器”，
            # 它在 `rsl_rl.modules.discriminator.Discriminator` 里已经实现。
            print(
                f"[DEBUG][AMP][Runner] Using baseline Discriminator for {alg_class_name} "
                f"(input_dim={amp_data.observation_dim * 2})"
            )
            discriminator = Discriminator(
                amp_data.observation_dim * 2,
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        # -- Episode info
        # 修改为每2次迭代输出一次，方便监控训练进度（必须与下面的打印频率一致）
        if locs['it'] % 10 == 0:
            ep_string = ""
            if locs["ep_infos"]:
                for key in locs["ep_infos"][0]:
        ...
        if locs['it'] % 10 == 0:  # 修改这个数字来调整打印频率（原始值：100）
            str = f" \033[1m Learning iteration {locs['it']}/{locs['tot_iter']} \033[0m "
