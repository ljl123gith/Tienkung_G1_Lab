...
# self.right_arm_dof_pos, 0, 1, 2, 3
# self.left_arm_dof_pos, 4, 5, 6, 7
# self.right_leg_dof_pos, 8, 9, 10, 11, 12, 13,
# self.left_leg_dof_pos, 14, 15, 16, 17, 18, 19
# self.right_arm_dof_vel, 20, 21, 22, 23
# self.left_arm_dof_vel, 24, 25, 26, 27
# self.right_leg_dof_vel, 28, 29, 30, 31, 32, 33
# self.left_leg_dof_vel, 34, 35, 36, 37, 38, 39
# left_hand_pos, 40, 41, 42
# right_hand_pos, 43, 44, 45
# left_foot_pos, 46, 47, 48
# right_foot_pos, 49, 50, 51


class AMPLoader_25:
    # 类变量作为默认值，实例变量会在__init__中根据实际数据更新
    JOINT_POS_SIZE = None  # 将在加载数据时自动检测
    JOINT_VEL_SIZE = None  # 将在加载数据时自动检测
    END_EFFECTOR_POS_SIZE = 0

    JOINT_POSE_START_IDX = 0
    JOINT_POSE_END_IDX = None  # 将在加载数据时计算

    JOINT_VEL_START_IDX = None  # 将在加载数据时计算
    JOINT_VEL_END_IDX = None    # 将在加载数据时计算

    END_POS_START_IDX = None  # 将在加载数据时计算
    END_POS_END_IDX = None    # 将在加载数据时计算
    ...
    def __init__(
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file) as f:
                ...
                # 第一次加载时自动检测数据维度
                if i == 0:
                    data_dim = motion_data.shape[1]
                    self._detect_data_format(data_dim)

                    print(f"[INFO] Detected motion data dimension: {data_dim}")
                    if data_dim == 52:
                        print(f"[INFO] Using 20DOF motion data format")
                    elif data_dim == 64:
                        # 原始 64 维，但我们会去掉前 6 维，最终 AMP 特征长度是 58
                        self.JOINT_POS_SIZE = 25          # 所有关节位置部分
                        self.JOINT_VEL_SIZE = 33          # 速度 + 脚高部分
                        self.END_EFFECTOR_POS_SIZE = 0
                        ...
                        self.END_POS_END_IDX = self.END_POS_START_IDX  # == 58
                        # keep_idx = np.arange(6, 64)   # 不要 root_pos(3)+root_euler(3)
                        # motion_data_reduced = motion_data[:, keep_idx]

                        print(f"[INFO] Using 25DOF GMR motion data format")
                    else:
                        print(f"[WARNING] Unknown motion data dimension: {data_dim}, attempting to load...")

                # 每个文件都要构造 motion_data_reduced
                if motion_data.shape[1] == 64:
                    keep_idx = np.arange(6, 64)        # 去掉 root_pos(3) + root_euler(3)
                    motion_data_reduced = motion_data[:, keep_idx]
                else:
                    motion_data_reduced = motion_data

                # Remove first 7 observation dimensions (root_pos and root_orn).
                self.trajectories.append(
                    torch.tensor(motion_data_reduced[:, : self.END_POS_END_IDX], dtype=torch.float32, device=device)
                )
                ...
                traj_len = (motion_data_reduced.shape[0] - 1) * frame_duration
                print(f"traj_len:{traj_len}")
                self.trajectory_lens.append(traj_len)
                self.trajectory_num_frames.append(float(motion_data_reduced.shape[0]))
    ...
    def _detect_data_format(self, data_dim):
        """根据数据维度自动设置格式参数
        
        支持的格式：
        - 52维 (20DOF): root_pos(3) + root_euler(3) + dof_pos(20) + root_lin_vel(3) + root_ang_vel(3) + dof_vel(20) = 52
        - 64维 (25DOF GMR): root_pos(3) + root_euler(3) + dof_pos(25) + root_lin_vel(3) + root_ang_vel(3) + dof_vel(25) + foot_heights(2) = 64
        """
        if data_dim == 52:
            # 20DOF 格式: 26 + 26 = 52
            self.JOINT_POS_SIZE = 26  # root_pos(3) + root_euler(3) + dof_pos(20)
            self.JOINT_VEL_SIZE = 26  # root_lin_vel(3) + root_ang_vel(3) + dof_vel(20)
            self.END_EFFECTOR_POS_SIZE = 0
        elif data_dim == 64:
            # 25DOF GMR 格式: 31 + 33 = 64
            self.JOINT_POS_SIZE = 31  # root_pos(3) + root_euler(3) + dof_pos(25)
            self.JOINT_VEL_SIZE = 33  # root_lin_vel(3) + root_ang_vel(3) + dof_vel(25) + foot_heights(2)
            self.END_EFFECTOR_POS_SIZE = 0
        else:
            # 通用处理：假设是 pos + vel 对称格式
            print(f"[WARNING] Unknown data dimension {data_dim}, using generic format detection")
            self.JOINT_POS_SIZE = data_dim // 2
            self.JOINT_VEL_SIZE = data_dim - self.JOINT_POS_SIZE
            self.END_EFFECTOR_POS_SIZE = 0
...
class AMPLoader:
    ...
    def __init__(
        ...
        time_between_frames: Amount of time in seconds between transition.
        motion_weights: Optional dictionary mapping filename (without extension) to weight.
        """
        self.device = device
        self.time_between_frames = time_between_frames
        ...
        # Resolve relative file paths robustly even if user runs scripts from outside TienKung-Lab/.
        # This file is located at: TienKung-Lab/rsl_rl/rsl_rl/utils/motion_loader.py
        # Project root (TienKung-Lab) is three levels up from this file.
        repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))

        for i, motion_file in enumerate(motion_files):
            self.trajectory_names.append(motion_file.split("/")[-1].split(".")[0])
            resolved_path = motion_file
            if not os.path.isabs(resolved_path) and not os.path.exists(resolved_path):
                # Common case: paths like "legged_lab/..." are relative to repo root.
            ...
            print(f"Loaded {traj_len}s. motion from {resolved_path}.")

        # Apply override weights if provided
        if motion_weights is not None:
            for i, name in enumerate(self.trajectory_names):
                if name in motion_weights:
                    print(f"Overriding weight for {name}: {motion_weights[name]}")
                    self.trajectory_weights[i] = motion_weights[name]
