...
def get_ankle_remove_indices(obs_dim):
    """根据观测维度自动返回需要移除的ankle关节索引
    
    Args:
        obs_dim: AMP观测维度 (52 for 20DOF, 64 for 25DOF)
        
    Returns:
        torch.Tensor: 需要移除的索引列表
    """
    # if obs_dim == 52:  # 20DOF format
    #     # 52维格式的ankle索引 (4个ankle位置 + 4个ankle速度)
    #     return torch.tensor([10, 13, 16, 19, 30, 33, 36, 39])
    # elif obs_dim == 64:  # 25DOF GMR format
    #     # 64维格式的ankle索引 (4个ankle位置 + 4个ankle速度)
    #     return torch.tensor([10, 11, 16, 17, 41, 42, 47, 48])
    # else:
    #     # 未知格式，不移除任何索引
    return torch.tensor([], dtype=torch.long)


class AMPPPO_Pairwise:
    ...
    def __init__(
        ...
        eta=0.2,  # HumanMimic soft boundary coefficient
        # RND parameters
        ...
        # Store HumanMimic soft boundary coefficient
        self.eta = eta
    ...
    def process_env_step(self, rewards, dones, infos, amp_obs):
        ...
        # 移除特定索引以匹配 amp_storage 和 amp_normalizer 的维度
        # 根据观测维度自动选择正确的ankle索引 (52维20DOF or 64维25DOF)
        obs_dim = self.amp_transition.observations.shape[1]
        remove_idx = get_ankle_remove_indices(obs_dim)
        keep_idx = torch.tensor([i for i in range(obs_dim) if i not in remove_idx.tolist()])
        amp_obs_reduced = self.amp_transition.observations[:, keep_idx]
        amp_obs_next_reduced = amp_obs[:, keep_idx]
        
        # record the transition
        # self.amp_storage.insert(amp_obs_reduced, amp_obs_next_reduced)
        # 不再做任何索引过滤
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            # 移除expert数据中的特定索引（从amp_data获取的是原始52维数据）
            # 根据观测维度自动选择正确的ankle索引 (52维20DOF or 64维25DOF)
            # obs_dim = expert_state.shape[1]
            # remove_idx = get_ankle_remove_indices(obs_dim)
            # keep_idx = torch.tensor([i for i in range(obs_dim) if i not in remove_idx.tolist()])
            # expert_state = expert_state[:, keep_idx]
            # expert_next_state = expert_next_state[:, keep_idx]
            ...
            #grad_pen_loss = self.discriminator.compute_grad_pen(*sample_amp_expert, lambda_=10)  #12.29
...
class AMPPPO_WGAN_GP:
    """
    Proximal Policy Optimization algorithm with WGAN-GP discriminator.
    """
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            # --- HumanMimic 修改: 调用 Soft-Boundary W-Loss ---
            # 原来的 logsigmoid 是 Pairwise Discriminator 特有的。
            # 我们现在改用你在 Discriminator 类里新写的 compute_humanmimic_loss
            
           
            amp_loss = self.discriminator.compute_humanmimic_loss(expert_d, policy_d, eta=self.eta)
