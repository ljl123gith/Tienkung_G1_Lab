...
from legged_lab.envs.g1.g1_config_25 import G1FlatEnvCfg_25, G1RoughEnvCfg_25
from legged_lab.utils.env_utils.scene import SceneCfg
from rsl_rl.env import VecEnv


class G1Env_25(VecEnv):
    """
    ...
    def __init__(
        self,
        cfg: G1FlatEnvCfg_25 | G1RoughEnvCfg_25 ,
        headless: bool,
    ):
        self.cfg: G1FlatEnvCfg_25 | G1RoughEnvCfg_25 
    ...
    def init_buffers(self):
        ...
        # ------------------------------------------------------------------
        # Optional: gait-phase buffers (用于步态相位参考观察；参考 g1_walkamp 设计)
        # ------------------------------------------------------------------
        # 我们通过 cfg.use_gait_obs 控制是否启用 gait-phase 观测，这样：
        # - 对于 g1_flat_25：在 G1FlatEnvCfg_25 中将 use_gait_obs=True，即可开启；
        # - 如果未来某个任务不希望使用 gait，相应 cfg 可以把 use_gait_obs 设为 False。
        self.use_gait_obs = getattr(self.cfg, "use_gait_obs", False)

        if self.use_gait_obs and hasattr(self.cfg, "gait"):
            # 步态相位：shape = (num_envs, 2)，分别表示左右腿在 [0, 1) 中的相位
            self.gait_phase = torch.zeros(
                self.num_envs, 2, dtype=torch.float, device=self.device, requires_grad=False
            )
            # 步态周期（秒），按 env 维度展开方便后续广播
            ...
            # 简单调试日志：只在环境初始化时打印一次，便于确认 gait-phase 设置是否生效
            print(
                "[G1Env_25] Gait-phase obs enabled for 25-DoF model: "
                f"cycle={self.cfg.gait.gait_cycle}, "
                f"air_ratio=({self.cfg.gait.gait_air_ratio_l}, {self.cfg.gait.gait_air_ratio_r}), "
                f"phase_offset=({self.cfg.gait.gait_phase_offset_l}, {self.cfg.gait.gait_phase_offset_r})"
            )

            # Helper buffers for gait rewards (requires feet_cfg initialized in super().init_buffers)
            # 这些缓冲区用于步态奖励函数（gait_feet_frc_perio, gait_feet_spd_perio 等）
            # 参考 g1_walkamp_env.py 的设计
    ...
    def compute_current_observations(self):
        ...
        如果 cfg.use_gait_obs=True，则会在原有 obs 的基础上追加 gait-phase 观测：
        [sin(phase_L), sin(phase_R), cos(phase_L), cos(phase_R), air_ratio_L, air_ratio_R] 共 6 维。
        """
        robot = self.robot
        ...
        # ------------------------------------------------------------------
        # Optional: append gait-phase observations to actor / critic
        # ------------------------------------------------------------------
        if getattr(self, "use_gait_obs", False):
            # gait_obs: shape = (num_envs, 6)
            gait_obs = torch.cat(
                [
                    torch.sin(2 * torch.pi * self.gait_phase),  # 2 维：sin(phase_L/R)
                    torch.cos(2 * torch.pi * self.gait_phase),  # 2 维：cos(phase_L/R)
                    self.phase_ratio,  # 2 维：air_ratio_L/R
                ],
                dim=-1,
            )
            current_actor_obs = torch.cat([current_actor_obs, gait_obs], dim=-1)
            current_critic_obs = torch.cat([current_critic_obs, gait_obs], dim=-1)

        return current_actor_obs, current_critic_obs

    def _calculate_gait_para(self) -> None:
        """
        根据当前 episode 时间更新步态相位。

        说明：
        - 使用 episode_length_buf * step_dt 作为时间轴；
        - 除以 gait_cycle 得到归一化时间，再加上左右腿各自的相位偏移；
        - 通过 mod 1.0 保证相位落在 [0, 1)。
        """
        if not getattr(self, "use_gait_obs", False):
            return

        # t: shape = (num_envs,)，单位为“步态周期个数”
        t = self.episode_length_buf * self.step_dt / self.gait_cycle
        self.gait_phase[:, 0] = (t + self.phase_offset[:, 0]) % 1.0
        self.gait_phase[:, 1] = (t + self.phase_offset[:, 1]) % 1.0

        # 调试：每隔若干 step 打印一次第 0 个 env 的相位，便于观察训练过程中的步态演化
        if (self.episode_length_buf[0] > 0) and (self.episode_length_buf[0] % 500 == 0):
            print(f"[G1Env_25] gait_phase[0]={self.gait_phase[0].tolist()}")
    ...
    def step(self, actions: torch.Tensor):
        ...
        # 如果启用了 gait 奖励，需要计算每步的平均脚部接触力和速度（用于步态周期性奖励）
        if getattr(self, "use_gait_obs", False):
            self.avg_feet_force_per_step = torch.zeros(
                self.num_envs, len(self.feet_cfg.body_ids), dtype=torch.float, device=self.device, requires_grad=False
            )
        ...
        for _ in range(self.cfg.sim.decimation):
            ...
            # 如果启用了 gait 奖励，在每个 physics step 中累加脚部接触力和速度
            # 参考 g1_walkamp_env.py 的设计
            if getattr(self, "use_gait_obs", False):
                # Calculate feet force and speed for gait rewards
        ...
        # 更新 gait-phase（如果启用），确保步态相位与 episode 时间同步
